...
CUBOID_URDF = join_paths(MODEL_DIRECTORY, 'drake/objects/simple_cuboid.urdf')
#####################################
...
# No randomness
def define_placement_on_aabb_aligned(
        top_body,
        bottom_body,
        bottom_aabb,
        top_pose=unit_pose(),
    ...
    """
    bottom_body의 실제 orientation을 가져와서,
    X/Y 중심 위에, Z축 방향으로 쌓되,
    top_body가 bottom_body와 같은 orientation을 갖도록 합니다.
    """
    # 1) bottom AABB의 X/Y 중심 계산
    lower = np.array(bottom_aabb[0])
    upper = np.array(bottom_aabb[1])
    center_xy = (lower[:2] + upper[:2]) / 2.0

    # 2) top_body 높이 반값
    _, extent = get_center_extent(top_body)
    half_z = extent[2] / 2.0

    # 3) Z 위치 = bottom top + half of top_body height + epsilon
    z = upper[2] + half_z + epsilon
    point = np.array([center_xy[0], center_xy[1], z])

    # 4) bottom_body의 실제 orientation 가져오기
    bottom_pose = get_pose(bottom_body)
    bottom_quat = bottom_pose[1]  # 혹은 bottom_pose.rotation

    # 5) bottom orientation + top_pose를 곱해서 최종 Pose 구성
    base = (point, bottom_quat)
    final_pose = multiply(base, top_pose)

    # 6) 시뮬레이터에도 반영
    set_pose(top_body, final_pose)
...
def sample_placement_half(top_body, bottom_body, bottom_link=None, right_half=False, **kwargs):
    aabb = get_aabb(bottom_body, link=bottom_link)

    min_pt = aabb.lower
    max_pt = aabb.upper
    mid_y = (min_pt[1] * 1 + max_pt[1] * 1) / 2.0

    if right_half:  # right half
        new_lower = (min_pt[0], mid_y, min_pt[2])
        new_upper = max_pt
        side_aabb = AABB(lower=new_lower, upper=new_upper)
    else:  # left half
        new_upper = (max_pt[0], mid_y, max_pt[2])
        new_lower = min_pt
        side_aabb = AABB(lower=new_lower, upper=new_upper)

    # closer to the robot
    min_pt = side_aabb.lower
    max_pt = side_aabb.upper
    mid_x = (min_pt[0] * 1 + max_pt[0] * 4) / 5.0

    new_lower = min_pt
    new_upper = (mid_x, max_pt[1], max_pt[2])
    bottom_aabb = AABB(lower=new_lower, upper=new_upper)

    return sample_placement_on_aabb(top_body, bottom_aabb, percent=2.0, **kwargs)


# No randomness
def define_placement(top_body, bottom_body, bottom_link=None, **kwargs):
    bottom_aabb = get_aabb(bottom_body, link=bottom_link)
    return define_placement_on_aabb_aligned(top_body, bottom_body, bottom_aabb, **kwargs)
