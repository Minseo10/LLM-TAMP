...
def dump_world():
    print("enter dump_world")
    for body in get_bodies():
        dump_body(body)
        print()
    print("exit dump_world")
...
def assign_link_colors(body, max_colors=3, alpha=1., s=0.5, colors=None):
    # TODO: graph coloring
...
def inverse_kinematics(robot, link, target_pose, max_iterations=200, max_time=INF, custom_limits={}, **kwargs):
    ...
    print("here we are in inverse kinematics")
    for iteration in irange(max_iterations):
        ...
        print("here we have kinematic conf")
        if kinematic_conf is None:
            print("kinematic conf is none")
            return None
    ...
    if not all_between(lower_limits, kinematic_conf, upper_limits):
        print("we are not between")
        print(lower_limits, upper_limits)
        print(kinematic_conf)
        return None
    print("we are returning kinematic conf")
    return kinematic_conf
